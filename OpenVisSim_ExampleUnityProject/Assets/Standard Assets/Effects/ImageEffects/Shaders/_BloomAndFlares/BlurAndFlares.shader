...
Shader "Hidden/BlurAndFlares" {
	...
	v2f_blur vertWithMultiCoords2 (appdata_img v) {
		...
		o.uv01 =  o.uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1);
		o.uv23 =  o.uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 2.0;
		o.uv45 =  o.uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 3.0;
		o.uv67 =  o.uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 4.0;
		o.uv67 =  o.uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 5.0;
		return o;  
	}
	...
	v2f_opts vertStretch (appdata_img v) {
		...
		o.uv[1] = o.uv[0].xy + b * 2.0 * _Offsets.xy;
		o.uv[2] = o.uv[0].xy - b * 2.0 * _Offsets.xy;
		o.uv[3] = o.uv[0].xy + b * 4.0 * _Offsets.xy;
		o.uv[4] = o.uv[0].xy - b * 4.0 * _Offsets.xy;
		o.uv[5] = o.uv[0].xy + b * 6.0 * _Offsets.xy;
		o.uv[6] = o.uv[0].xy - b * 6.0 * _Offsets.xy;
		return o;
	}
	...
	v2f_opts vertWithMultiCoords (appdata_img v) {
		...
		o.pos = UnityObjectToClipPos(v.vertex);
		o.uv[0] = TRANSFORM_TEX(v.texcoord, _MainTex);
		o.uv[1] = o.uv[0].xy + 0.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[2] = o.uv[0].xy - 0.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[3] = o.uv[0].xy + 1.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[4] = o.uv[0].xy - 1.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[5] = o.uv[0].xy + 2.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[6] = o.uv[0].xy - 2.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		return o;
	}	
