...
half4 frag( v2f i ) : SV_Target
{
    ...
    half4 geom = tex2D (VRWorksGetDepthNormalsSampler(), VRWorksRemapUV(i.uv));
    
    for (int s = 0; s < NUM_BLUR_SAMPLES; ++s)
    {
        float2 nuv = i.uv + o * (s+1);
        half4 ngeom = tex2D (VRWorksGetDepthNormalsSampler(), VRWorksRemapUV(nuv.xy));
...
v2f vert (appdata_img v)
{
	v2f o;
	o.pos = UnityObjectToClipPos (v.vertex);
	o.uv[0] = TRANSFORM_TEX(v.texcoord, _MainTex);//MultiplyUV (UNITY_MATRIX_TEXTURE0, v.texcoord);
	o.uv[1] = TRANSFORM_TEX(v.texcoord, _SSAO);//MultiplyUV (UNITY_MATRIX_TEXTURE1, v.texcoord);
	return o;
}
