 ...
 Shader "Hidden/CameraMotionBlurDX11" {
  ...
  float4 _MainTex_ST;
	...
	{	
		...
		float zx = SAMPLE_DEPTH_TEXTURE(VRWorksGetDepthSampler(), VRWorksRemapUV(x));
		...
		for(int l=0; l<NUM_SAMPLES; l++) 
		{
			...
			float zy = SAMPLE_DEPTH_TEXTURE_LOD(VRWorksGetDepthSampler(), float4(VRWorksRemapUV(y),0,0));
			zy = -Linear01Depth(zy);
